...
HRESULT YYUTManager::ChangeDevice(YYUTD3D9DeviceSettings* new_device_Settings, bool force_recreate, bool clip_window_to_single_adpater)
{
#ifdef _DEBUG
	...
	if(!force_recreate &&CanDeviceBeReset(old_d3ddevice_setting,new_device_Settings))
	{
		...
		if(FAILED(hr))
		{
			...
			else 
			{
				...
				if(FAILED(ChangeDevice(new_device_Settings,true,clip_window_to_single_adpater)))
				{
					ShutDown();
				}
				else
...
//
//
HRESULT YYUTManager::SetD3D9DeviceCursor(HCURSOR hCursor, bool bpush_backWatermark)
{
	HRESULT hr=E_FAIL;
	ICONINFO icon_info;
...
HRESULT YYUTManager::Create3DEnvironment9()
{
	TRACE_FUNCTION
	HRESULT hr=S_OK;
	IDirect3DDevice9 *d3d_device=GetD3D9Device();
...
{
	TRACE_FUNCTION
	YYUT::YYSetConsole(GetHWNDFocus());
	if(GetD3D9()==NULL)
	{
	...
	if(FindVaildDeviceSettings(&device_setting))
	{
		...
		throw exception("can't find a property device setting");
	HRESULT hr=ChangeDevice(&device_setting,false,true);
	if(FAILED(hr))
...
//	HRESULT hr;
	...
	{
		if(FAILED(ChangeDevice(&dev_setting,false,false)))
		{
			if(FAILED(ChangeDevice(&old_device_settings,false,false)))
				throw exception("toggle full screen error");
		}
